...
class JabberBot(object):
    ...
    def connect( self):
        if not self.conn:
            ...
            conn.RegisterHandler( 'iq', self.callback_iq)
            conn.sendInitPresence(requestRoster=True)
            self.conn = conn
    ...
    def callback_iq( self, conn, mess):
        """Defined to handle subscription requests. Always accepts."""

        print mess
#        if mess.getType() == 'set' and mess.getID() == 'push':
#            for item in mess.getQueryChildren():
#                try:
...
#                else:
#                    if sub == 'none':
#                        self.conn.Roster.Authorize(item.getAttr('jid'))
#                        print 'autorizou'

        if mess.getType() == 'result' and mess.getID() == '3':
            for item in mess.getQueryChildren():
                try:
                    sub = item.getAttr('subscription')
                except AttributeError:
